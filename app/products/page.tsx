  ...
  return (
    <main className="min-h-screen bg-black text-white">
      ...
      <div className="border-b border-gray-800 p-8">
        <h1 className="text-4xl font-bold tracking-widest mb-2">PRODUCTS</h1>
        <p className="text-gray-400 text-xs tracking-widest">{products.length} products</p>
      </div>
      <div className="max-w-7xl mx-auto p-8">
        {products.length===0?(
          <div className="text-center py-24"><p className="text-gray-500 text-xs tracking-widest">No products yet</p></div>
        ):(
              ...
              return (
                <Link key={p.id} href={`/product/${p.id}`} className="group cursor-pointer">
                  <div className="bg-gray-900 rounded aspect-square mb-3 overflow-hidden">
                    {img?<img src={img} alt={p.name} className="w-full h-full object-cover group-hover:scale-105 transition duration-500"/>:<div className="w-full h-full flex items-center justify-center text-gray-700 text-xs tracking-widest">NO IMAGE</div>}
                  </div>
