  ...
  useEffect(() => {
    ...
    if (cached) {
      try {
        ...
        const converted = (amountINR * loc.rate).toFixed(2);
    ...
    fetch("/api/location")
      ...
      .catch(() => {
        setDisplay(`₹${amountINR.toFixed(2)}`);
        setReady(true);
      });
  }, [amountINR]);
  ...
  return (
    <span
      ...
      style={{
        ...
        color: strikethrough ? "#6b7280" : undefined,
      }}
    >
      {display || `₹${amountINR.toFixed(2)}`}
    </span>
  );
}
